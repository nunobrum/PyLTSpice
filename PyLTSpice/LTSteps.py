...
class LTSpiceExport(object):
    ...
    The data is then accessible by using the following attributes implemented in this class.
        ...
        + headers:      => list containing the headers on the exported data
        + dataset:      => dictionary in which the keys are the the headers and the export file and the values are
    ...
    **Examples***
    ...
    ::
        ...
        for value in export_data.dataset['I(V1)']:
            p+int(f"Do something with this value {value}")
...
class LTSpiceLogReader(object):
    ...
    **Properties**
       ...
       + stepset:       => dictionary in which the keys are the variables that were STEP'ed during the simulation and
                        the associated value is a list representing the sequence of assigned values during simulation.

        + headers:      => list containing the headers on the exported data. This is only populated when the
                        'read_measures' optional parameter is set to False.

        + dataset:      => dictionary in which the keys are the the headers and the export file and the values are
                           lists. This is information is only populated when the 'read_measures' optional parameter is
                           set to False.
